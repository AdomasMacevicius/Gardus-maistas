...
mealsGroup.MapGet("meals", async (SiteDbContext siteDbContext, CancellationToken cancellationToken, int restaurantId,
    int menuId) =>
{
    Restaurant? restaurant = await siteDbContext.Restaurants
        .FirstOrDefaultAsync(restaurant => restaurant.Id == restaurantId);
...
mealsGroup.MapGet("meals/{mealId}", async (SiteDbContext siteDbContext, int restaurantId, int menuId, int mealId) =>
{
    Restaurant? restaurant = await siteDbContext.Restaurants
        .FirstOrDefaultAsync(restaurant => restaurant.Id == restaurantId);
    Menu? menu = await siteDbContext.Menus
    ...
    int restaurantId, int menuId) =>
{
    ...
    return Results.Created($"/api/restaurants/{restaurant.Id}/menus/{menu.Id}", new CreateMealDto(meal.Id, meal.Name,
        meal.Description, meal.Price));
});

mealsGroup.MapPut("meals/{mealId}", async (SiteDbContext siteDbContext, int restaurantId, int menuId, int mealId,
...
mealsGroup.MapDelete("meals/{mealId}", async (SiteDbContext siteDbContext, int restaurantId, int menuId, int mealId) =>
{
    Restaurant? restaurant = await siteDbContext.Restaurants
        .FirstOrDefaultAsync(restaurant => restaurant.Id == restaurantId);
    Menu? menu = await siteDbContext.Menus
